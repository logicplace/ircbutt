...
module.exports = function Memory(settings) {
	...
	this.installPackets = function installPackets(list, header) {
		...
		if ("matrix" in list) {
			for (var x in list.matrix) {
				...
				p.context = "matrix";
				installedPackets.push(_.defaults(p, defaultPacket));
			}
		}
	...
	this.packet = function packet(command, context) {
		...
		//     "context": heading/context this packet falls under
		// }
		...
		for (var i = 0; i < command.length; ++i) {
			...
			if (_.isObject(command[i])) {
				_.assign(lookup, _.omit(command[i], "name"));
				if ("name" in command[i]) name = command[i].name;
			} else {
				name = command[i];
			}
			if (context) _.defaults(lookup, { "context": context });
			...
			if (name) {
				...
				results = _.filter(installedPackets, lookup);
			}
			...
			for (var j = 0; j < results.length; ++j) {
				...
				result.event = name || (result.numeric == "000" ? result.id : result.numeric) || result.id;
	...
	this.my = function my(property, value) {
		...
		if (assignedNick) {
			...
			nicknameIndex = _.indexOf(myInfo.nicknames, myInfo.nickname);
		}
	...
	// Users and accounts
	var accounts = {};
	this.account = function account(username) {
		// BOILER
		return {
			"add": function (data) {
			...
			"login": function (password) {
				return username in accounts && accounts[username].password == password;
			},

			"permissions": function (what) {
				return username in accounts && { "all": true, "access": true };
			},
		}
	}
