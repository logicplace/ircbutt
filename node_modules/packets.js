...
function compilePacket(packet) {
	...
	while (packet.length) {
		var angle = indexOfNonEscaped(packet, "<"),
		    ...
		    order = [angle, brace, lbrack, lparen, rbrack, rparen, slash].sort(compareNumbers)
		...
		for (var i = 0; i < order.length; i++) {
			...
			curStruct.out += packet.substr(0, order[i]).replace("%", "%%");
			...
			case slash:
				...
				switch (packet.charAt(groupName.length)) {
				case ":": break;
