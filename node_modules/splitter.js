...
module.exports = Class({
	...
	init: function(spl,globalRefs,caseInsensitive){
		...
		,subGroups=[[]],namespace=[],nsLimit=[],curNS=-1,ns;
		for(i=0;i<len;++i){
			c = spl.charAt(i);
			...
			if(wasgrp){
				...
				var token = [nom,tore,mod,i,pre,ns];
				if(orArray){
					orArray.push(token);
					orArray = null;
				} else {
		...
		for(var i=1;i<m.length;i++){
			...
			if(i in groups && res){
				if("token" in groups[i]){
					var arr = [],tmp;
					nom = numIdx++;
					var re = new RegExp("^("+groups[i].token[1]+")("+groups[i].token[4]+"|$)");
			...
			if(i%2 == 0){
				...
				for(j=0;j<opts.length;++j){
					...
					if(Array.isArray(tk[0])){
						...
						if(tk[2].indexOf("*") != -1){
							var tmp,dni=(numIdx in data)?data[numIdx++]:[];
							for(k=0;k<dni.length;++k){
						...
						if(tk[5]){
							...
							for(var j=0;j<nss.length;++j){
								if(!(nss[j] in ns)){
									throw new AssertionFailed(
										nss.slice(0,j).join(".")+" namespace","Nothing",tk[3]
									);
								}
								ns = ns[nss[j]];
							}
