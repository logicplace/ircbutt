...
function applySplitter_findEnd(spltr,i,find){
	...
	return [str.replace(new RegExp("%("+find+")","g"),"$1"),mod,end+xtra,pre];
}
...
module.exports = Class({
	...
	init: function(spl,globalRefs,caseInsensitive){
		...
		,subGroups=[[]],namespace=[],nsLimit=[],curNS=-1,tkByName={};
		for(i=0;i<len;++i){
			c = spl.charAt(i);
			...
			switch(c){
				case "<": { //Spaceless group
					...
					if(grp.charAt(0) == "#"){
						...
						if(pt.length == 1){
							tore = "[^ "+pre+"]+";
						} else { arg=pt[2]; switch(pt[1]){
							case ":": {
								//These are not allowed as well as space
								tore = "[^ "+pre+escapeRE4Set(arg,true)+"]+";
								break;
							} case "=": {
								//Only these are allowed
								if(arg.slice(-1) == "+"){
									tore = "["+escapeRE4Set(arg,true).slice(0,-1)+"]+";
								} else {
									tore = "["+escapeRE4Set(arg,true)+"]";
								}
								break;
							} case "!": {
								...
								//Reference
								if(arg.charAt(0) == "_"){
									if(!(arg in globalRefs)){
										throw new AssertionFailed(
											arg+" in global definitions","Not found",i
										);
									}
									var struct = globalRefs[arg],stFirst = struct.charAt(0)
									, matches = {"<":">","{":"}","/":"/"},grp2;
									...
									//TODO: Stay within namespace?
									//TODO: Possibly add addressing by index
									if(arg in tkByName){
										var tmptk = tkByName[arg].slice(-1)[0]
										tore = tmptk[1];
										if(mod=="")mod = tmptk[2];
										if(pre=="")pre = tmptk[4];
										break;
									...
									if(tore == ""){
										throw new AssertionFailed(
											arg+" to be defined","Was not defined",i
										);
									}
								}
								break;
					...
					} else { var arg=pt[2]; switch(pt[1]){
						case ":": {
							if(!arg.match(/^[0-9]+$/)){
								throw new AssertionFailed("Number for argument",arg,i);
							}
							if(pre=="")pre = " ";
							tore = "[^"+pre+"]+("+pre+"[^"+pre+"]+){0,"+arg+"}";
							break;
						} case "=": {
							if(!arg.match(/^[0-9]+$/)){
								throw new AssertionFailed("Number for argument",arg,i);
							...
							if(pre=="")pre = " ";
							tore = "[^"+pre+"]+("+pre+"[^"+pre+"]+){"+arg+"}";
							break;
						} case "?": {
							var right = -1, innerMod = "", lb = "(", rb = ")", struct;
							if(arg.charAt(0) == "?"){
								arg = arg.slice(1);
								innerMod = "?";
							}
							if(arg.charAt(0) == "<")arg = arg.slice(1);
							while((right=arg.indexOf(">",right+1)) != -1 && arg.charAt(right-1) == "%");
							if(right != -1){
								var tmpr = arg.slice(right+1);
								arg = arg.slice(0,right)
								right = tmpr;
							} else right = "";
							arg = arg.replace(/%(.)/g,"$1");
							right = right.replace(/%(.)/g,"$1");
							/*if(right)tore = "(.+)"+escapeRE(arg)+"|([^ ]+)";
							else tore = escapeRE(arg)+"(.+)|([^ ]+)";*/
							arg = arg.replace(/[<>\[\](){}\/]/g,"%$0");
							right = right.replace(/[<>\[\](){}\/]/g,"%$0");
							if(mod.indexOf("?") != -1){lb="[";rb="]";}
							struct = lb+arg+"{"+nom+"}"+innerMod+right+rb+"|"+lb+"<"+nom+">"+rb;
							mod = mod.replace("?","");
							if(mod)struct = "("+struct+")"+mod;
							spl = spl.slice(0,i)+struct+spl.slice(ni+1);
							len = spl.length;
							ni = i-1;
							nom = null;
			...
			if(wasgrp){
				...
				if(!(nom in tkByName))tkByName[nom] = [];
				tkByName[nom].push(token);
				if(orArray){
					orArray.push(token);
					orArray = null;
				} else {
	...
	//TODO: Somehow determine the specificness of matches to choose between in a set
	//eg. in "({ha})|(<ha>)" <ha> is the most specific and if it matches, that form should
	//be written to.
	_write: function(data,except,tokens){
		...
		for(i=0;i<tokens.length;++i){
			...
			if(i%2 == 0){
				...
				//Group
				var opts = tokens[i],optional=0,errs=[];
				...
				for(j=0;j<opts.length;++j){
					...
					if(Array.isArray(tk[0])){
						...
						if(tk[0] in ns){
							...
							var d = ns[tk[0]],cont=false;
							if(typeof(d) == "string")d = [d];
							for(k=0;k<d.length;++k){
								if(!d[k].match("^("+tk[1]+")$")){
									errs.push(new AssertionFailed(tk[1],d[k],tk[3]));
									if(tk[2].indexOf("?") != -1)++optional;
									cont = true;
									break;
								}
							}
				...
				if(j == opts.length && optional != j){
					//TODO: Throw all? Somehow?
