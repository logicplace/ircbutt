...
//TODO: Add handlers on how to split these over multiple messages

var Splitter = require("./splitter.js");

var SERVER=1,CLIENT=2,msgs;

function splitWholeEntries(len,data){
	var rlen = 0;
	for(var i=0;i<data.length && (rlen+=data[i].length) <= len;++i);
...
function splitFormattedText(len,data){
	...
	,   pos,fmt;
	if(ret.match(/\1/g).length % 2 == 1){
		//CTCP was cut off
		pos = ret.lastIndexOf("\1");
		rmdr = ret.substr(pos)+rmdr;
		ret = ret.substr(0,pos);
	}
	fmt = ret.match(/[\2\3\26\37\17]/g);
...
module.exports = msgs = {
	...
	"005": {
		sc: SERVER,
		msg: [
			"<nick> <support>* :are supported by this server", //When sent at the start (more common)
			"<nick> :Try server <server>, port <#port>", //RPL_BOUNCE (RFC2812)
		],
		split: [
