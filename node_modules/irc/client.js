...
module.exports = function Client(settings) {
	...
	this.connect = function connect(connectedHandler) {
		function init () {
			...
			self.connection.on("initial", self.connection.reader(["numeric", "server"]));
			...
			self.connection.on("numeric", function (data) {
				...
				if (data.inlay) _(data).assign(self.connection.read(data.numeric, data.inlay));
			...
			self.connection.on("server", function (data) {
				// Parse the sender for more useful info
				data.sender = self.connection.read("target", data.sender) || data.sender;

				// Try to parse the command and mix it into the data
				if (data.inlay) _(data).assign(self.connection.read(data.command, data.inlay));
			...
			self.connection.on("PING", pong, self.connection);
			...
			self.connection.on("PRIVMSG", function (data) {
				data.event = "PRIVMSG(" + data.nickname +")";
				return data;
			});
			self.connection.on("NOTICE", function (data) {
				data.event = "NOTICE(" + data.nickname +")";
				return data;
			});
