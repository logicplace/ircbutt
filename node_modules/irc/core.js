		...
		function handleData(event, data) {
			...
			if (event in handlers) {
				...
				for (var i = 0; i < evhandlers.length; ++i) {
					if (evhandlers[i].passive) {
						// If this is passive, it can be done async
						setTimeout((function (handler) {
							return function () { handler.handler(data, handler.userdata) }
						})(_.clone(evhandlers[i])), 1);
					} else {
		...
		var out = {
			...
			"collect": function (options, handler) {
				...
				function setupCollection() {
					//irc.info("Setting up collection " + collectionID.toString())
					if (options.start) {
						//irc.info("  " + options.start)
				...
				function afterStart(data, fromStart) {
					...
					//irc.info("Starting timer for collection " + collectionID.toString())
					ending = setTimeout(timedOut, options.timeout);
				}

				function collectEvent(data, event) {
					if (started) {
						//irc.info("Collected event " + event + " for " + collectionID.toString())
				...
				function timedOut() {
					//irc.info("Collection timed out for " + collectionID.toString())
					c.timedOut = true;
					deregisterCollection();
				}
