...
module.exports = Class({
	...
	_modPath: function(module) {
		...
		return path.normalize(path.join(this.botPath, 'modules', module + '.js'));
	},
	...
	//Socket commands
	send: function(command,data,priority,sock){
		if(command in proto){
			var proCom = proto[base.command],msg;
			...
			var mode = this.server?"_server":"_client",raw;
			...
			if(this.server){
				...
				if("target" in data){
					var m;
					if(m=data.target.match(/([#+&]|![A-Z0-9]{5})([a-z]+)/)){
						//Channel
					} else if(m=data.target.match(/([a-zA-Z\[\]^`]+)/)){
	...
	sendRaw: function(str,priority,sock){
